...
public class Solution {
    ...
    private void pathSum(TreeNode root, int sum, LinkedList<Integer> pathList) {
        if (root == null) return;
        int currentValue = (pathList.isEmpty()) ? root.val : pathList.getLast() + root.val;
        for (int i : pathList) {
            if (currentValue - i == sum) sumNum++;
        }
        if (currentValue == sum) sumNum++;
        pathList.addLast(currentValue);
        pathSum(root.left, sum, pathList);
        pathSum(root.right, sum, pathList);
        pathList.removeLast();
    }
}